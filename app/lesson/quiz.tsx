...
export const Quiz = ({
  ...
  const [matchCompleted, setMatchCompleted] = useState(false); // for MATCH
  ...
  const onMatchComplete = (matchedPairs: string[]) => {
    if (matchedPairs.length === options.length / 2) {
      setStatus("correct");
  ...
  const onContinue = () => {
    if (challenge.type === "MATCH" && !matchCompleted) return;
    if (!selectedOption && challenge.type !== "MATCH") return;
  ...
  return (
    <>
      ...
      <Footer
        disabled={
          pending ||
          (challenge.type === "MATCH" && !matchCompleted) ||
          (!selectedOption && challenge.type !== "MATCH")
        }
        status={status}
        onCheck={onContinue}
      />
